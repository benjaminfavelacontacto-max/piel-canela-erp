...
import { Plus, Trash2, X, Loader2, Home, PackagePlus } from "lucide-react"
import { crearSalidaInterna } from "./actions"

type ProductoBase = { id: string; sku: string; nombre: string }
type Fila = { producto_id: string; sku: string; nombre: string; cantidad: number }
...
export function NuevaSalida({ productos }: { productos: ProductoBase[] }) {
  ...
  const agregar = (p: ProductoBase) => {
    ...
    setFilas((prev) => [...prev, { producto_id: p.id, sku: p.sku, nombre: p.nombre, cantidad: 1 }])
    setBusqueda("")
  }
      ...
      <div className="mb-3 flex items-center justify-between gap-2">
        <div>
          ...
          <p className="text-[11.5px] text-gray-500">
            Registra lo que se lleva Piel Canela. <b>Descuenta inventario</b> y NO cuenta en finanzas.
          </p>
        </div>
        <button
          type="button"
              ...
              return (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="text-[10.5px] text-gray-500">{p.sku}</p>
                  </div>
