...
- **Por producto** (`cotizacion_items` / `venta_items`): la partida guarda el precio YA REBAJADO en `precio_unitario`, el de catálogo congelado en `precio_lista` y CÓMO se capturó en `descuento_tipo` (`pct` | `monto`) + `descuento_valor` (15 = 15%, 500 = **$500 sobre la partida completa**, que se reparten entre sus piezas). Como el precio rebajado es el que se persiste, `subtotal` (GENERATED), `cotizaciones.subtotal`, el IVA, la utilidad y la venta espejo cuadran solos — no hay que tocar ninguna columna generada.
- ⚠️ **Cambiar la cantidad de una partida con descuento en monto OBLIGA a recalcular su `precio_unitario`** (`conCantidad()` en `src/lib/descuentos.ts`): $500 entre 4 piezas no es lo mismo que entre 8. Mutar solo `cantidad` congela el descuento — el mismo bug que tuvo el descuento global porcentual antes del fix de 2026-07-27. Un monto que no divide exacto ($100 entre 3) deja el ahorro real en $99.99: la fuente de verdad es `precio_unitario`, nunca el valor tecleado.
